...
class OptimizationSMPL(nn.Module):
    def __init__(self):
        ...
        self.pose = nn.Parameter(torch.zeros(1, 45).cuda())
        self.beta = nn.Parameter((torch.zeros(1, 10).cuda()-0.5/100))
...
#        return NetworksFactory.get_by_name('voxel_encoder_and_predictor3', 2048, 2+14, input_dim=7, b_min = np.array([-0.8, -0.8, -0.8]), b_max = np.array([0.8, 0.8, 0.8]))
        return NetworksFactory.get_by_name('voxel_encoder_and_predictor3', 2048, 1+16, input_dim=7, b_min = np.array([-1.2, -1.2, -1.2]), b_max = np.array([1.2,1.2,1.2]))
        #return NetworksFactory.get_by_name('voxel_encoder_and_predictor3', 2048, 778*3, input_dim=7, b_min = np.array([-1.2, -1.2, -1.2]), b_max = np.array([1.2,1.2,1.2]))
...
#        self._input_voxels = torch.clamp(self._input_voxels, 0, clamp)*multiplier
    ...
    def forward(self, keep_data_for_visuals=False, interpolate=0, resolution=128):
        if not self._is_train:
            ...
            with torch.no_grad():
                ...
                body_class_pred = pred[:,1:].unsqueeze(1).permute(0, 1, 3, 2)
        ...
        if True:
            with torch.no_grad():
                ...
                try:
                    ...
                    body_verts = body_verts / resolution*2.4 - 1.2
    ...
    def get_current_visuals(self):
        ...
        #trim_mesh = trimesh.Trimesh(self.pred_mesh[0].cpu().data.numpy(), self.MANO.th_faces.cpu().data.numpy(), process=False)
        trim_mesh = self.pred_mesh
        trim_mesh.visual.vertex_colors[:, :3] = [160, 160, 255]
        scene = pyrender.Scene(ambient_light=[.1, .1, .3], bg_color=(0, 0, 0))
        pyrender_mesh = pyrender.Mesh.from_trimesh(trim_mesh, smooth=False)
